...
StatusType osekEvent_SetEvent(T_OSEK_TASK_ControlBlock *tcbPtr, EventMaskType mask)
{
    ...
    if( tcbPtr->status == ((TaskStateType)1) )
    {
...
StatusType  GetEvent( TaskType taskId, EventMaskRefType mask )
{
	...
	if( (osekTask_TaskTable[taskId].configTable.property & ( (OSBYTE)0x80 )) == 0 )
	{
		return ( (StatusType)1 ) ;
	}

	
	if( osekTask_TaskTable[taskId].status == ((TaskStateType)3) )
	{
		return ( (StatusType)7 ) ;
	}
